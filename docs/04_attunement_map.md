...
この位置から 観察 または 判断保留 が再び参照される遷移が記述される。

位置の再同定が行われた時点で、乱れは記述可能な位置として扱われる。
...
HAS において、この地図は熟練（Mastery）を前提としない。
地図との関係性が反復的に参照されることが想定されている。
